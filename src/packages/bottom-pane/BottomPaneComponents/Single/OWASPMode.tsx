    ...
    <div className="h-full bg-[#050505] flex flex-col overflow-hidden">
      <div className="flex items-center px-6 py-4 border-b border-zinc-900 bg-[#0a0a0a] justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-sm font-black text-white uppercase tracking-tighter">OWASP Security Auditor</h2>
            <div className="text-[9px] text-zinc-500 font-bold uppercase tracking-widest">Top 10 Vulnerability Scanner</div>
          </div>
        </div>
        ...
        {analyzing && (
            <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black text-red-500 uppercase tracking-widest animate-pulse">Scanning...</span>
            </div>
        )}
      </div>
            ...
            {findings.map((finding) => (
                <div key={finding.id} className="group bg-zinc-900/40 border border-zinc-800 rounded-2xl p-5 hover:border-red-500/30 transition-all duration-300 relative overflow-hidden">
                    ...
                    <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-2">
                            <span className="text-[9px] font-black bg-zinc-800 text-zinc-500 px-2 py-0.5 rounded uppercase">{finding.id}</span>
                            <span className={`text-[9px] font-black px-2 py-0.5 rounded uppercase ${
                                finding.severity === 'critical' ? 'bg-red-950/50 text-red-400' : 
                                finding.severity === 'high' ? 'bg-orange-950/50 text-orange-400' : 'bg-yellow-950/50 text-yellow-400'
                            }`}>{finding.severity}</span>
                        </div>
                        <span className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest">{finding.category}</span>
                    </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-zinc-800/50 flex justify-between items-center opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button className="text-[9px] font-black text-red-500 uppercase tracking-widest hover:text-red-400">View Remediation Guide</button>
                    </div>
                </div>
            ))}
        </div>
...
const Placeholder = ({ text }: { text: string }) => (
    <div className="h-full flex flex-col items-center justify-center bg-[#050505] p-10 text-center">
      <div className="w-20 h-20 rounded-full bg-red-600/5 flex items-center justify-center text-red-950 mb-6 border border-red-950/10">
        <svg className="w-10 h-10 opacity-20" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M12 15v2m0 0v3m0-3h3m-3 0H9m12 1a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
      </div>
