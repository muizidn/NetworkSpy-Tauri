            ...
            <div className="grid grid-cols-2 gap-3">
                <div className="bg-black/20 p-2.5 rounded-xl border border-white/5">
                    <div className="text-[8px] font-black text-zinc-600 uppercase tracking-widest mb-1 flex items-center gap-1">
                        {finding.isError ? <FiAlertTriangle size={8} /> : <FiInfo size={8} />} {finding.isError ? "Cause" : "Metadata"}
                    </div>
                    <div className="text-[10px] text-zinc-400 italic font-medium leading-tight">{finding.risk}</div>
                </div>
                <div className="bg-black/20 p-2.5 rounded-xl border border-white/5">
                    <div className="text-[8px] font-black text-zinc-600 uppercase tracking-widest mb-1 flex items-center gap-1">
                        {finding.isError ? <FiEdit2 size={8} /> : <FiCheckCircle size={8} />} {finding.isError ? "Fix" : "Output"}
                    </div>
...
export const CodeSnippetMode = () => {
  ...
  const { customFindings, isRunning } = useCustomScripts<CodeSnippetFinding>('code_snippet', trafficData.body, trafficData.headers);
      ...
      <div className="flex-grow overflow-y-auto no-scrollbar pb-10">
        {activeTab === "Custom Scripts" ? (
            <div className="max-w-4xl mx-auto p-4 @sm:p-8">
                <CustomScriptManager category="code_snippet" />
            </div>
        ) : (
