  ...
  return (
    <div className="h-full bg-[#050505] flex flex-col overflow-hidden">
      <div className="flex items-center px-6 py-4 border-b border-zinc-900 bg-[#0a0a0a] justify-between">
        ...
        {!isFirebase && data && (
            <div className="px-3 py-1 bg-red-950/30 border border-red-900/50 rounded-full">
...
const DetailRow = ({ label, value, isCode }: { label: string, value: string, isCode?: boolean }) => (
    <div className="flex justify-between items-center text-[11px]">
        ...
        <span className={`${isCode ? 'font-mono bg-zinc-800/50 px-1.5 py-0.5 rounded' : 'font-bold'} text-zinc-200`}>{value}</span>
    </div>
);
...
const Placeholder = ({ text }: { text: string }) => (
    <div className="h-full flex flex-col items-center justify-center bg-[#050505] p-10 text-center">
      <div className="w-20 h-20 rounded-full bg-orange-600/5 flex items-center justify-center text-orange-950 mb-6 border border-orange-950/10">
        <SiFirebase className="w-10 h-10 opacity-20" />
      </div>
      <h3 className="text-zinc-400 font-bold mb-1 italic">Firebase Inspector</h3>
