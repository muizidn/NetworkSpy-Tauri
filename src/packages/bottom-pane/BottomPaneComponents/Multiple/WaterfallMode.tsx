    ...
    return (
      <div className="h-full bg-[#1e1e1e] flex flex-col overflow-hidden">
        <div className="p-4 border-b border-zinc-800 bg-zinc-900/50 flex justify-between items-center">
            <h2 className="text-sm font-bold text-zinc-300 uppercase tracking-widest">Network Waterfall</h2>
            <div className="text-[10px] text-zinc-500 font-mono">Total Time: {maxTime.toFixed(0)} ms</div>
        </div>
                    ...
                    return (
                        <div key={i} className="flex items-center group">
                            <div className="w-48 shrink-0 flex flex-col pr-4">
                                <span className="text-[10px] font-mono text-zinc-400 truncate">{String(item.method)} {String(item.url).split('/').pop()}</span>
                                <span className="text-[9px] text-zinc-600 truncate">{String(item.url)}</span>
                            </div>
                            <div className="flex-grow bg-zinc-900/30 h-6 rounded flex items-center relative overflow-hidden group-hover:bg-zinc-800/50 transition-colors">
                                {/* Timeline Grid Lines */}
